...
public class YHttpOrderInterfaces extends BaseInterface{

	
	private DocumentBuilder builder;
	public YHttpOrderInterfaces(Context context, Handler handler) {
	...
	public Object parseJSONXML(String jsonOrXmlStr) {
		...
		try {
			...
			for (int i = 0; i < pageArray.length(); i++) {
				...
				hashMap.put("HotelName", orderJsonObject.get("DepCity").toString());
				hashMap.put("CheckInDate", orderJsonObject.get("RentCarTime").toString());
				hashMap.put("OrderId", orderJsonObject.get("Id").toString());
				hashMap.put("TotalPrice", orderJsonObject.get("TotalAmount").toString());
